...
async function api(path,options){let r=await fetch(path,options),text=await r.text(),value;try{value=JSON.parse(text)}catch(_){value=null}if(!r.ok)throw new Error(value?.error||text||('HTTP '+r.status));return value}function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}function setStatus(element,state,message){element.className='portal-status'+(state?' '+state:'');element.textContent=message||''}async function withBusy(form,label,work){let button=form.querySelector('button[type="submit"],button:not([type])'),original=button.textContent;button.disabled=true;button.textContent=label;try{return await work()}finally{button.disabled=false;button.textContent=original}}
...
function deploymentCommands(sequence,releaseType,install){let actions=['check-update','download-update'];if(install)actions.push('activate-update');return actions.map(action=>({action:action,release_sequence:sequence,release_type:releaseType}))}
async function refreshDeploymentChoices(){let data=await api('api/releases'),items=data.inventory.releases.filter(r=>r.verified&&(r.channels||[]).length),controls=[document.getElementById('deployment-release'),document.getElementById('rollout-release')],typeLabels={application:'Application',firmware:'Core',universal:'Universal'};for(const control of controls){let selected=control.value;control.innerHTML='';for(const release of items){let channel=release.channels[0];for(const releaseType of ['universal','application','firmware']){if(!release.assets?.[releaseType])continue;let option=document.createElement('option');option.value=String(release.release_sequence)+':'+releaseType;option.dataset.sequence=String(release.release_sequence);option.dataset.releaseType=releaseType;option.dataset.channel=channel;option.textContent=(release.version||release.tag)+' · '+typeLabels[releaseType]+' · '+channel;control.append(option)}}if(selected&&[...control.options].some(option=>option.value===selected))control.value=selected;if(!control.options.length){let option=document.createElement('option');option.textContent='Promote a verified update first';option.value='';control.append(option);control.disabled=true}else control.disabled=false}}
...
async function refreshDeploymentState(){let button=document.getElementById('deployment-submit'),release=document.getElementById('deployment-release'),device=document.getElementById('policy-device'),selected=release?.selectedOptions[0],sequence=Number(selected?.dataset.sequence||0),releaseType=selected?.dataset.releaseType||'';if(!button||!release||!device||!sequence||!releaseType){if(button)button.disabled=true;return}try{let data=await api('api/devices'),record=data.devices.find(item=>item.id===device.value),pending=record?.fleet?.pending_commands||[],matching=pending.filter(command=>Number(command.release_sequence)===sequence&&(!command.release_type||command.release_type===releaseType));button.disabled=matching.length>0;button.textContent=matching.length?'Update queued':'Deploy update';button.title=matching.length?matching.length+' update command'+(matching.length===1?'':'s')+' pending on device':''}catch(_){button.disabled=false;button.textContent='Deploy update'}}
document.getElementById('policy').onsubmit=async event=>{event.preventDefault();let form=event.target,values=Object.fromEntries(new FormData(form)),release=document.getElementById('deployment-release').selectedOptions[0],sequence=Number(release.dataset.sequence||0),releaseType=release.dataset.releaseType||'',install=values.deployment_action==='install',status=document.getElementById('deployment-status');if(!sequence||!releaseType){setStatus(status,'error','Promote a verified update before deploying.');return}let request={device_id:values.device_id,channel:release.dataset.channel,start_time:values.start_time,end_time:values.end_time,maximum_failures:Number(values.maximum_failures),automatic_download:true,automatic_activation:install,commands:deploymentCommands(sequence,releaseType,install)};setStatus(status,'','Creating signed deployment…');try{await withBusy(form,'Deploying…',async()=>{let response=await api('api/policy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(request)}),pending=(response.pending_commands||[]).filter(command=>Number(command.release_sequence)===sequence&&command.release_type===releaseType).length;setStatus(status,'success',(release.textContent||'Update')+' queued for '+document.getElementById('policy-device').selectedOptions[0].textContent+'. '+pending+' command'+(pending===1?'':'s')+' pending.');await refresh()});await refreshDeploymentState()}catch(error){setStatus(status,'error',error.message)}}
document.getElementById('rollout').onsubmit=async event=>{event.preventDefault();let form=event.target,values=Object.fromEntries(new FormData(form)),release=document.getElementById('rollout-release').selectedOptions[0],status=document.getElementById('rollout-status'),request={release_sequence:Number(release.dataset.sequence||0),release_type:release.dataset.releaseType||'',channel:release.dataset.channel,maximum_failures:Number(values.maximum_failures),cohorts:values.cohorts.split(',').map(value=>value.trim()).filter(Boolean)};setStatus(status,'','Creating controlled deployment…');try{await withBusy(form,'Creating…',async()=>{let rollout=await api('api/rollouts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(request)});setStatus(status,'success','Controlled deployment '+rollout.id+' is ready. Dispatch the active cohort when ready.');await refresh()})}catch(error){setStatus(status,'error',error.message)}}
window.rolloutAction=async function(id,action){let status=document.getElementById('rollout-status');try{await api('api/rollouts/'+action,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})});setStatus(status,'success',action==='dispatch'?'Active cohort dispatched.':'Deployment advanced to the next cohort.');await refresh()}catch(error){setStatus(status,'error',error.message)}}
document.getElementById('profile-editor').onsubmit=async event=>{event.preventDefault();let form=event.target,values=Object.fromEntries(new FormData(form)),settings={timezone_name:values.timezone_name,ntp_servers:values.ntp_servers.split(',').map(value=>value.trim()).filter(Boolean),loglevel:values.loglevel,log_buffer_lines:Number(values.log_buffer_lines),ha_discovery:form.elements.ha_discovery.checked,ha_discovery_prefix:values.ha_discovery_prefix,mqtt_enabled:form.elements.mqtt_enabled.checked,mqtt_server:values.mqtt_server,mqtt_port:Number(values.mqtt_port),mqtt_username:values.mqtt_username,mqtt_base_topic:values.mqtt_base_topic,mqtt_qos:Number(values.mqtt_qos),syslog_enabled:form.elements.syslog_enabled.checked,syslog_host:values.syslog_host,syslog_port:Number(values.syslog_port),syslog_transport:values.syslog_transport},request={format_version:1,name:values.name,description:values.description,settings:settings},status=document.getElementById('profile-status');setStatus(status,'','Saving profile…');try{await withBusy(form,'Saving…',async()=>{let profile=await api('api/profiles',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(request)});setStatus(status,'success','Profile '+profile.name+' saved.');await refreshProfiles()})}catch(error){setStatus(status,'error',error.message)}}
document.getElementById('profile-apply').onsubmit=async event=>{event.preventDefault();let form=event.target,values=Object.fromEntries(new FormData(form)),status=document.getElementById('profile-apply-status');setStatus(status,'','Applying profile…');try{await withBusy(form,'Applying…',async()=>{await api('api/profiles/apply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(values)});setStatus(status,'success','Profile '+values.profile+' applied to '+document.getElementById('profile-device').selectedOptions[0].textContent+'.')})}catch(error){setStatus(status,'error',error.message)}}
