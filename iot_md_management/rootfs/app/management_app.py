...
.button.danger,button.danger{border-color:var(--bad);background:transparent;color:var(--bad)}.button.danger:hover,button.danger:hover{background:var(--bad);color:#fff}
...
<div data-page-section="devices"><section class="hero"><p class="eyebrow">Fleet</p><h1>Devices</h1><p>Register mutually authenticated IoT-MD devices and inspect their latest reported state.</p></section><div id="devices" class="cards"></div><section class="card"><p class="eyebrow">Enrollment</p><h2>Register device</h2><form id="register"><div class="cards"><label>ID<input name="id" placeholder="e.g. IoT-MD-002" required></label><label>Name<input name="name" placeholder="e.g. Workshop controller"></label><label>Host<input name="host" placeholder="e.g. iot-md-002.local (without https://)" required></label><label>Port<input name="port" type="number" value="8444" placeholder="8444"></label><label>CA path<input name="ca_path" value="/ssl/iot-md-ca.pem" placeholder="/ssl/iot-md-ca.pem" required></label><label>Client certificate path<input name="cert_path" value="/ssl/iot-md-fleet.pem" placeholder="/ssl/iot-md-fleet.pem" required></label><label>Client key path<input name="key_path" value="/ssl/iot-md-fleet-key.pem" placeholder="/ssl/iot-md-fleet-key.pem" required></label><label>Cohort<input name="cohort" value="default" placeholder="e.g. default or canary"></label></div><button>Register</button></form></section></div>
...
async function pollDevice(id){await api('api/poll',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({device_id:id})});await refresh()}
