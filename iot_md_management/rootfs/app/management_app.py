...
async function refreshDeploymentState(){let button=document.getElementById('deployment-submit'),release=document.getElementById('deployment-release'),device=document.getElementById('policy-device');if(!button||!release||!device||!Number(release.value)){if(button)button.disabled=true;return}try{let data=await api('api/devices'),record=data.devices.find(item=>item.id===device.value),pending=record?.fleet?.pending_commands||[],sequence=Number(release.value),matching=pending.filter(command=>Number(command.release_sequence)===sequence);button.disabled=matching.length>0;button.textContent=matching.length?'Deployment queued':'Deploy release';button.title=matching.length?matching.length+' deployment command'+(matching.length===1?'':'s')+' pending on device':''}catch(_){button.disabled=false;button.textContent='Deploy release'}}
document.getElementById('policy').onsubmit=async event=>{event.preventDefault();let form=event.target,values=Object.fromEntries(new FormData(form)),release=document.getElementById('deployment-release').selectedOptions[0],sequence=Number(release.value),install=values.deployment_action==='install',status=document.getElementById('deployment-status');if(!sequence){setStatus(status,'error','Promote a verified release before deploying.');return}let request={device_id:values.device_id,channel:release.dataset.channel,start_time:values.start_time,end_time:values.end_time,maximum_failures:Number(values.maximum_failures),automatic_download:true,automatic_activation:install,commands:deploymentCommands(sequence,install)};setStatus(status,'','Creating signed deployment…');try{await withBusy(form,'Deploying…',async()=>{let response=await api('api/policy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(request)}),pending=(response.pending_commands||[]).filter(command=>Number(command.release_sequence)===sequence).length;setStatus(status,'success',(release.textContent||'Release')+' queued for '+document.getElementById('policy-device').selectedOptions[0].textContent+'. '+pending+' command'+(pending===1?'':'s')+' pending.');await refresh()});await refreshDeploymentState()}catch(error){setStatus(status,'error',error.message)}}
document.getElementById('rollout').onsubmit=async event=>{event.preventDefault();let form=event.target,values=Object.fromEntries(new FormData(form)),release=document.getElementById('rollout-release').selectedOptions[0],status=document.getElementById('rollout-status'),request={release_sequence:Number(release.value),channel:release.dataset.channel,maximum_failures:Number(values.maximum_failures),cohorts:values.cohorts.split(',').map(value=>value.trim()).filter(Boolean)};setStatus(status,'','Creating controlled deployment…');try{await withBusy(form,'Creating…',async()=>{let rollout=await api('api/rollouts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(request)});setStatus(status,'success','Controlled deployment '+rollout.id+' is ready. Dispatch the active cohort when ready.');await refresh()})}catch(error){setStatus(status,'error',error.message)}}
